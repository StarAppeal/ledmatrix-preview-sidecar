...
## Overview
This service bridges WebSocket clients and a Python backend for LED matrix preview frames. It:
- Accepts WebSocket connections from clients on port 8765.
- Receives raw RGB frames from Python over TCP on port 5001.
- Forwards client events/commands to Python over TCP on port 5002.
...
- TCP command broadcaster forwards client events to the Python process.
...
Subsequent client messages are forwarded to Python as JSON (see below).

### TCP frames (Python -> service) on port 5001
Binary protocol:
...
### TCP commands (service -> Python) on port 5002
